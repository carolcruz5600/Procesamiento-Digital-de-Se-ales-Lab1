...
Inicialmente, se crea un array lleno de 0 con la cantidad de datos $n$ de nuestra señal con ``np.zeros(n)``, luego se crea un nuevo array ``indices`` con una cantidad aleatoria de ``1`` en un array de ``0`` de tamaño $n$ con ``np.random.choice`` y ``p`` representa la probabilidad de aparecer de cada número, donde finalmente habrán más ``0`` que ``1``, simbolizando los impulsos. Por último, se multiplica ``indices`` con un nuevo array que contiene ``1`` y ``-1`` simbolizando impulsos positivos y negativos con una amplitud de ``1.5``. La variable ``ruido_impulso`` es la encargada de guardar el ruido impulso, y ``señal_impulso`` es donde se guarda la señal contaminada por el Ruido Impulso.
...
"""
Editor de Spyder
Parte B y C del Laboratorio 1 de PDS
"""
...
#%%
#Parte B
    #Parámetros de la Señal Capturada
fs=1000 #frec. de muestreo
n=10000 #numero de muestras
T=n/fs #duración de la señal
t = np.linspace(0, T, n, endpoint=False) #vector tiempo
#%%
    #Captura de la Señal
import nidaqmx
...
##np.savetxt(f"Parte_A{fs}.txt",data)
#%%
#Asignar señal guardada a variable data
data=np.loadtxt('C:/Users/carlo/.spyder-py3/Parte_A1000.txt')

#Graficar la señal
plt.plot(data)
plt.xlabel(r'Número de Dato (n)')
plt.ylabel(r'Amplitud (V)')
plt.title('Señal ECG')
plt.show()

#%%
   #Calculo valores estadísticos
media=np.mean(data)
desviacion_estandar=np.std(data,ddof=1)
kurtosis_scipy=kurtosis(data)
c_var=variation(data)
print('------ Valores Estadísticos ------')
print('Media:',round(media,3))
print('Desviación Estándar:',round(desviacion_estandar,3))
print('Coeficiente de Variación:',round(c_var,3))
print('Curtosis:',round(kurtosis_scipy,3))
print('----------------------------------')
#%%
    #Histograma
plt.hist(data,bins=35,edgecolor='black',color='red')
plt.title("Histograma ECG")
plt.xlabel(r"Voltaje (V)")
plt.ylabel(r"Frecuencia de Aparición")
plt.show()
    #Curtosis en Histograma
sns.histplot(data, kde=True, bins=35,color='red')
plt.title("Curtosis sobre Histograma")
plt.xlabel(r"Voltaje (V)")
plt.ylabel(r"Frecuencia de Aparición")
plt.show()
    #Función de Probabilidad
plt.hist(data,density=True,bins=35,edgecolor='black', color='red')
...
#%%
#Parte C
    #Generación de Señales Contaminadas
    
        #Ruido Gaussiano
ruido_gaussiano = np.random.normal(0,0.2,n)
señal_gaussiano = data + ruido_gaussiano

        #Ruido Impulso
ruido_impulso = np.zeros(n)
indices = np.random.choice([0, 1], size=n, p=[0.99, 0.01])
ruido_impulso = indices * (np.random.choice([-1, 1], size=n) * 1.5)
señal_impulso = data + ruido_impulso

        #Ruido Artefacto
ruido_artefacto = 0.4*np.sin(2*np.pi*60*t)
señal_artefacto = data + ruido_artefacto

    #Potencias
pot_señal = np.mean(data**2) #Señal original
pot_gaussiano = np.mean((señal_gaussiano - data)**2) #Gaussiano
pot_impulso = np.mean((señal_impulso - data)**2) #Impulso
pot_artefacto = np.mean((señal_artefacto - data)**2) #Artefacto

    #Cálculos de SNR
SNR_gauss = 10 * np.log10(pot_señal / pot_gaussiano)
SNR_impulso = 10 * np.log10(pot_señal / pot_impulso)
SNR_artefacto = 10 * np.log10(pot_señal / pot_artefacto)

#Mostrar los datos
print('--------SNRs--------')
print('Gaussiano = ',round(SNR_gauss,2), "dB")
print('Impulso = ',round(SNR_impulso,2), "dB")
print('Artefacto = ',round(SNR_artefacto,2), "dB")
print('--------------------')
